...
public class RNGPGSAuth extends ReactContextBaseJavaModule {
  ...
  // To be exposed to JS
  public final static String AUTH_STATE_CHANGE_EVENT =
   ...
   * @param triggerUI whether or not to trigger the interactive UI if silent
   *     sign in fails.
   * @param promise
   */
  @ReactMethod
  ...
  public void signInPlayerWithUI(final Promise promise) {
    if (!this.isSignedIn()) {
      ...
      getCurrentActivity().startActivityForResult(this.startSignInIntent(),
                                                  RQC_SIGNIN_UI);
    } else {
      Helpers.sendAuthStateChangedEvent(getReactApplicationContext(), true);
      Helpers.resolvePromise(promise);
   ...
   */
  public boolean isSignedIn() { return getSignedInUser() != null; }

  private Intent startSignInIntent() {
    //        GoogleSignInClient signInClient =
    //        GoogleSignIn.getClient(getReactApplicationContext(),
    //                GoogleSignInOptions.DEFAULT_GAMES_SIGN_IN);

    // get sign in client with special scope for saved games (feature coming
    // soon)
  ...
  private void handleSignInActivityResults(int requestCode, int resultCode,
                                           Intent data) {
    ...
    if (result.isSuccess()) {
      ...
      Helpers.rejectPromise(mSignInWithUIPromise,
                            new Exception("Sign In Failed."));
    }
  }
